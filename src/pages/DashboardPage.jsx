...
const DashboardPage = () => {
  ...
  // Flatten all components across all ships
  const allComponents = ships.flatMap(ship => ship.components || []);

  // Helper: calculate if a component is overdue (no maintenance or last maintenance > DAYS_OVERDUE)
  ...
  // Flatten all jobs
  const allJobs = allComponents.flatMap(comp => comp.maintenance || []);

  const jobsInProgress = allJobs.filter(job => job.status === "In Progress").length;
  const jobsCompleted = allJobs.filter(job => job.status === "Completed").length;
